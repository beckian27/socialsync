...
struct CreateInvite: View {
    ...
    @State var endtime: Date = Date(timeIntervalSinceNow: 9999999)
...
//                }
                ...
                Form{
                    ...
                    Button("Add Another Timeslot") {
                        var end = Calendar.current.dateComponents([.day, .month,.year,.hour,.minute], from: date)
                        let temp = Calendar.current.dateComponents([.hour,.minute], from: endtime)
                        end.hour = temp.hour
                        end.minute = temp.minute
                        
                        let endfromcomps = Calendar.current.date(from:end)!
                        print(endfromcomps, end)
                        
                        let interval = DateInterval(start: date, end: endfromcomps)
                        print(interval)
